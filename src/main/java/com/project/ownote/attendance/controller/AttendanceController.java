...
//        }
    ...
    public String recordAttendancePage(HttpSession session, Model model) {
        ...
        if (!attendanceService.findByEmpNumAAndAtt_date((long) authInfo.getEmp_num(), currentDate).isEmpty()) {
            ...
            LocalTime localTime = LocalTime.parse(currentTimeFormatted, formatter);
            attendanceService.getAttendanceById(attendance.getAttendance_id());
            attendance.setAtt_ontime(localTime);
    ...
    public String recordAttendancePageP(HttpSession session, Model model) {
        ...
        if (attendanceService.findByEmpNumAAndAtt_date((long) authInfo.getEmp_num(), currentDate).get(0).getAtt_offtime() != null) {
            ...
            // Format the 퇴근시간 (leave time) to include minutes
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("HH:mm");
            String formattedLeaveTime = currentTime.format(formatter);
            attendance.setAtt_offtime(LocalTime.parse(formattedLeaveTime, formatter));
